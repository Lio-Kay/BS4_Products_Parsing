...
def main() -> None:
    """
    Создаем логгер, хедер, получаем главную страницу через gather_main_page_data
    на её основе парсим категории продуктов через gather_each_page_data
    """

    # Настраиваем логгер
    logging.basicConfig(level=logging.DEBUG, format='%(asctime)s %(levelname)s:%(message)s',
                        filename='logs.txt', filemode='w', encoding='utf-8')
    logging.debug(f'Запустили скрипт')
    ...
    print(f'Закончили парсинг за {emd_main-start_main:.2f} сек, данные сохранены в папке data, логи в logs.txt')


def gather_main_page_data(headers: dict, url: str, retry: int = 3, sleep_time: int = 10) -> dict:
    """
    ...
    try:
        ...
        if retry:
            logging.error(f'Попытка:{retry}, статус: {requests.get(url=url, headers=headers).status_code}')
            # Если все плохо - приостанавливаем код
            time.sleep(sleep_time)
            # Возвращаем функцию, уменьшив кол-во попыток, и увеличив время ожидания
            return gather_main_page_data(headers, url, retry=(retry - 1), sleep_time=(sleep_time+30))
        else:
            raise ex
    ...
    # Создаем на его основе .json, просто для наглядности
    with open(file='all_categories_dict.json', mode='w', encoding='utf-8') as file:
        json.dump(obj=all_categories_dict, fp=file, indent=4, ensure_ascii=False)

    end = time.perf_counter()
    print(f'Собрали данные главной страницы и добавили ссылки на категории в файл'
          f' all_categories_dict.json за {end-start:.2f} сек.')

    return all_categories_dict


def gather_each_page_data(counter: int, category: str, category_url: str, headers: dict) -> None:
    """
    ...
    try:
        ...
        # Записываем в csv
        with open(file=f'data/{counter}.{category_fixed}.csv', mode='w', encoding='utf-8') as file:
        ...
        product_table = soup.find(class_='mzr-tc-group-table').find('tbody').find_all('tr')
        for item in product_table:
            ...
            with open(file=f'data/{counter}.{category_fixed}.csv', mode='a', encoding='utf-8') as file:
                file.write(f'{title},{calories},{proteins},{fats},{carbohydrates}\n')

        logging.debug(f'Закончили собирать данные из категории №{counter}: {category}')
    # Если отсутствует одна из страниц с данными, логируем и пропускаем её
    except AttributeError as exc:
        logging.error(f'Ошибка AttributeError {exc} в категории №{counter}: {category}')
        pass
